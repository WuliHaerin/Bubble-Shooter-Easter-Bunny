...
public class bonus : MonoBehaviour {
	...
	void Start () {
		...
		//	
		//	.MoveTo(gameObject,iTween.Hash("position", new Vector3(transform.position.x-8, transform.position.y, transform.localScale.z), "time",3f,"easetype",iTween.EaseType.easeInOutSine));
		}
		else{
			transform.position = new Vector3(-3,  transform.position.y, -10f);
	...
	IEnumerator flyUp(){
		...
		.Hash("y", -0.2f, "time",0.2f,"easetype",iTween.EaseType.easeInOutSine));
                yield return new WaitForSeconds(0.2f);
            }*/
	}
